...
```{r, echo=F}
gam.4 <- gam(Salary ~ Category + Class + nStudents + gpaAverage + Year + Level, data = df)

gam.4.df = augment(gam.4)

ggplot(data = gam.4.df, aes(x = .fitted, y = Salary)) + geom_point() + ggtitle("Response vs. Fitted") + geom_smooth(method = 'loess')
ggplot(data = gam.4.df, aes(x = .resid)) + geom_histogram()

#print(gam.4$coefficients)
```
...
To model the relationship between salary and our feature variables of interest, a generalized additive model (GAM) was fitted on the data. As we can see from the plot of Salary vs. the GAM's fitted values, the model does an adequate job of prediction on the data. The trend of this graph seems to be nearly linear. In addition, the distribution of the model's residuals is relatively close to normal.
Based on our model, it does not seem that GPA has any sort of influential impact on Salary. 

```{r, echo = F}
gam.5 <- gam(Salary ~ Category + Class + nStudents + Year + Level, data = df)

gam.5.df = augment(gam.5)

ggplot(data = gam.5.df, aes(x = .fitted, y = Salary)) + geom_point() + ggtitle("Response vs. Fitted") + geom_smooth(method = 'loess')
ggplot(data = gam.5.df, aes(x = .resid)) + geom_histogram()

#print(gam.5$coefficients)
```

After removing GPA from our model, the trend of the model's predictions vs. the actual response variable is still linear, and the residuals are still largely normally distributed. 
