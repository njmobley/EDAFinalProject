...
```
Based on these simple visualizations, from our data we learn that professors have the highest log salaries with lecturers earning the least. Associate and assistant professor appear to have a similar mean, as well as senior lecturers and lecturers. The density plot indicates that average GPA across all instructor levels centers close to 3.5. This may indicate that average GPA and instructor level do not significantly impact one another. The instructor level breakdown by course level shows there's more instructors in 300 level courses and the least in 400 level courses. It will be interesting to see if outcomes in 300 level courses are much different from the others due to the volume of instructors. The last visualization shows the breakdown of instructor levels against various departments. There does not appear to be a disinct pattern in instructor level against departments.
...
The next visualizations show the relationships between log of salaries and GPA over instructor level, department, and year. The purpose of these visualizations is to determine if there is a visible relationship between salaries and GPA.
...
This first graphic looks at log of salary and average GPA over through 2010-2018. It appears that salary increases at higher levels of GPA. It will be more beneficial to analyze other interest variables to determine the importance of year. Year alone cannot adequately determine if this increase in salary is related to increase in GPA. The next visualization also factors in instructor level for a deeper understanding of these relationships.
...
This shows the relationship between average GPA and salary over the nine year period by instructor level. We also learn that average GPA increased throughout the years for each instructor level. By examining salary on a log scale (the color) and average GPA (y-axis), there appears to be a positive relationship between them. For associate and assistant professors and senior lecturers, the color becomes lighter as it progresses throughout the year, indicating an increase in salary. Professors and lecturers appear to have some relationship with GPA and salary, but not as much compared to the other instructor levels. This could be indicative of a relationship between GPA and salaries. 
...
g4 <- df %>%
  ...
  ggplot(aes(x=aveGPA,y=log(aveSalary),color=Class)) +
    ...
    geom_smooth(method="rlm",se=FALSE) +
    scale_color_viridis_d() + 
    ggtitle("Log Salary Over GPA By Department") +
    xlab("GPA") +
    ylab("Log Salary") + 
    facet_wrap(~Category)
g4
...
First, five GAM's were constructed consisting of one variable and Salary as the predictor to calculate the explained variance. This determined feature importance.
...
```
This shows the explained variance across single variable GAM's. This shows the level of variance each variable captures when predicting salary. The single variable GAM's indicate that the order of most important variables for predicting salary are instructor level, department, course level, GPA, and year (from most to least influential). With regards to salary, on its own, GPA plays an insignificant role. Even though GPA on its own cannot predict or explain salary, it can be incorporated into a multi-variable gam model to optimize predictions. The next step is to incrementally add variables into the GAM and evaluate the results to determine the best model. The variables will be added in descending order of explained variance.
...
```{r visualizing-the-model, echo = F}
ggplot(data = gam4.df, aes(x = gpaAverage, y = .fitted, color = Class)) + geom_point() + scale_color_viridis_d() + geom_jitter() + ggtitle("Fitted vs. GPA")
```
